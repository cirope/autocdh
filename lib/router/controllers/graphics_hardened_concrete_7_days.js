var hardenedConcreteData = function () {
  ...
  var cracksCursor = Cracks.find({
    days: 7, stress: { $ne: null }, sampleId: { $in: sampleIds }, tubeType: { $ne: 'bending' }
  }, {
  ...
  if (cracks.length) {
    while (filter.start.isBefore(filter.end)) {
      ...
      _.each(_cracks, function (crack) {
        ...
        if (concreteNumber && dateDiff >= 6 && dateDiff <= 12) {
          var value = Math.round(crack.stress / +concreteNumber[0] * 100)
