...
var addSettlementLimitSeries = function (series) {
  if (series.length) {
    ...
    var upperLimit  = Array.apply(null, new Array(length)).map(Number.prototype.valueOf, 10)
    var lowerLimit  = Array.apply(null, new Array(length)).map(Number.prototype.valueOf, 5)
...
var hardenedConcreteData = function () {
  ...
  if (filterSamples(filter).count()) {
    while (filter.start.isBefore(filter.end)) {
      ...
      var cracks    = Cracks.find({ days: 28, stress: { $ne: null }, error: { $lte: 15 }, sampleId: { $in: sampleIds } })
      var serie     = -1
      var noValue   = -1
      var bySample  = {}
      var i         = 0
    ...
    if (strength.individualCriteria && strength.averageCriteria) {
      var upperLimit  = Array.apply(null, new Array(labels.length)).map(Number.prototype.valueOf, strength.individualCriteria)
      var lowerLimit  = Array.apply(null, new Array(labels.length)).map(Number.prototype.valueOf, strength.averageCriteria)

      series.unshift({
        className: 'ct-series ct-series-c only-line dotted-a',
        data: upperLimit
      })

      series.unshift({
        className: 'ct-series ct-series-d only-line dotted-b',
        data: lowerLimit
      })
    }
  }
