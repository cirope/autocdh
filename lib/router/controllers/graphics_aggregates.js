  ...
  if (Graphics.filterGranulometries(filter).count()) {
    while (filter.start.isBefore(filter.end)) {
      ...
      granulometries.forEach(function (granulometry) {
        var value = attribute === 'thin' ? granulometry.thin.percentage : granulometry[attribute]
...
GraphicAggregateFinenessController = RouteController.extend({
  data: function () { return data.call(this, 'fineness') }
})

GraphicAggregateThinController = RouteController.extend({
  data: function () { return data.call(this, 'thin') }
})
