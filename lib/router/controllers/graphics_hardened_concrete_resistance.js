...
var hardenedConcreteData = function () {
  ...
  if (strength && strength.individualCriteria && filter.discardHighErraticValues === 'true')
    cracksSelector.stress = { $ne: null, $lte: strength.individualCriteria + 15 }

  var cracksCursor    = Cracks.find(
    _.extend(_.clone(cracksSelector), { sampleId: { $in: _.pluck(samples, '_id') }, tubeType: { $ne: 'bending' } }),
