...
Schemas.Maintenance = new SimpleSchema([Schemas.Base, {
  ...
  plantId: {
    ...
    regEx: SimpleSchema.RegEx.Id,
    custom: function () {
      var instrumentId     = this.field('instrumentId').value
      var instrument       = instrumentId && Instruments.findOne(instrumentId)
      var shouldBeRequired = instrument && instrument.hasPlant

      if (shouldBeRequired && ! Plants.findOne(this.value)) return 'required'
    },
    autoform: {
      firstOption: false,
      options: function () {
...
if (Meteor.isClient) {
  Schemas.Maintenance.labels({
    ...
    fileId:          function () { return TAPi18n.__('maintenance_file') },
