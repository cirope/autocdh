...
var deviationData = function () {
  ...
  if (Graphics.filterSamples(filter).count()) {
    ...
    while (filter.start.isBefore(filter.end)) {
      ...
      var cracks     = Cracks.find({ stress: { $ne: null }, error: { $lte: 15 }, sampleId: { $in: sampleIds } })
      ...
      series[0].data.push(cracks.count() ? average : 0)
    ...
    if (strength.efficiencyDesirable && strength.efficiencyDesirableLow) {
...
GraphicConcreteEfficiencyController = RouteController.extend({
  data: function () { return deviationData.call(this) }
})
