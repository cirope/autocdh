...
// TODO: remove
Granulometries.setMaterial = function () {
  var materials = Materials.first()
  var options = {
    sand:   _.first(materials.sands),
    gravel: _.first(materials.gravels)
  }

  Granulometries.find().forEach(function (g) {
    var material = options[g.type]

    Granulometries.update(g._id, { $set: { type: g.type, materialId: material._id } })
  })
}
