...
var castQuery = function (query, sampleIds, defaultSelector) {
  ...
  if (+query.receipt) {
    var receipts  = Receipts.find({ number: +query.receipt })
  ...
  if (query.molding) selector.moldingIn = dateRange(moment(query.molding))
  if (query.cracked) selector.crackedIn = dateRange(moment(query.cracked))
...
CustomerCracksController = RouteController.extend({
  data: function () {
    ...
    var samplesSelector = { customerId: customer._id }
