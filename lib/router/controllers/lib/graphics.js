...
var filterByAdditions = function (additions, selector, options) {
  ...
  concretes.forEach(function (concrete) {
    var concretePartition = _.groupBy(concrete.concretes, function (c) {
      var _concrete  = _.find(materials.concretes, function (_c) {
        return _c._id === c.id
      })
      var isConcrete = /^C\w{1,3}\s*\d+$/i.test(_concrete.name.trim())

      return isConcrete && 'concretes' || 'additions'
    })

    var concreteSum  = _.reduce(concretePartition['concretes'], function (memo, c) {
      return memo + c.amount
    }, 0)
    var additionsSum = _.reduce(concretePartition['additions'], function (memo, c) {
      return memo + c.amount
    }, 0)
    var percentage  = additionsSum * 100 / concreteSum

    if (percentage >= limit.min && percentage <= limit.max)
      samples.push(concrete.sampleId)
  })

  selector._id = { $in: samples }
}
...
Graphics = {
  ...
  filterGranulometryHumidities: function (filter, attribute, options) {
    var selector = dateSelector(filter)
    ...
    selector[attribute] = { $ne: null }

    return GranulometryHumidities.find(selector, options)
  },
  ...
  deviation: function (values) {
    ...
    _.each(values, function (value) {
      deviations.push(Math.pow(value - mean, 2))
    })
