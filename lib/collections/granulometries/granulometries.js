Granulometries = new Mongo.Collection('granulometries')
...
var humidityPercentageFor = function (granulometry) {
  ...
  return netDry > 0 ? Math.abs((netWet - netDry) / netDry * 100) : 0
}
...
var thinPercentageFor = function (granulometry) {
  ...
  return netAfter > 0 ? Math.abs((netBefore - netAfter) / netBefore * 100) : 0
}
...
var finenessFor = function (granulometry) {
  ...
  if (granulometry.type === 'sand') {
    ...
    }, 300 - 3 * (granulometry.thin ? granulometry.thin.percentage : 0)) / 100
  }
...
var extraModifierAttributes = function (granulometry) {
  ...
  if (granulometry.humidity)
    modifier['humidity.percentage'] = humidityPercentageFor(granulometry)

  if (granulometry.thin) {
    granulometry.thin.percentage = thinPercentageFor(granulometry)
    modifier['thin.percentage']  = granulometry.thin.percentage
  }

  modifier['fineness'] = finenessFor(granulometry)
...
Meteor.methods({
  ...
  removeGranulometry: function (documentId) {
    var hasMixtures = Mixtures.find({ 'granulometries.id': documentId }).count()

    if (! hasMixtures) Granulometries.remove(documentId)
  }
})
