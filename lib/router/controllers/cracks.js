...
var castCrackedQuery = function (query) {
  ...
  if (query.strength) {
    var strengths = Strengths.find({ name: new RegExp('.*' + query.strength + '.*') })
    var concretes = Concretes.find({
      strengthId: { $in: strengths.map(function (s) { return s._id }) }
    })

    selector.sampleId = { $in: concretes.map(function (c) { return c.sampleId }) }
  }
  ...
  if (query.moldingIn) selector.moldingIn = dateRange(moment(query.moldingIn))
  if (query.crackedIn) selector.crackedIn = dateRange(moment(query.crackedIn))
