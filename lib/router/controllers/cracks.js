...
var castCracksQuery = function (query) {
  ...
  if (query.customer) {
    ...
    if (sampleIds.length){
      selector.sampleId = { $in: sampleIds }
    }
  }
...
var castCrackedQuery = function (query) {
  ...
  if (ids.length) selector.sampleId = { $in: ids }
...
CrackEditController = RouteController.extend({
  data: function () {
    ...
    if (crack && lastCracked && !crack.stress) {
      if(!crack.pressId && lastCracked.pressId) {
        _.extend(crack, {
          pressId: lastCracked.pressId
        })
      }
      if(!crack.responsibleId && lastCracked.responsibleId){
        _.extend(crack, {
